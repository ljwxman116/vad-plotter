...
## Usage
```
python vad.py RADAR_ID [ -m STORM_MOTION ] [ -s SFC_ID ] [ -t TIME ] [ -f IMG_NAME ] [ -p LOCAL_PATH ]
```
...
* `SFC_ID` is the station ID to get the surface wind vector. Ex: krfd or KRFD. (Will be adding a separate arg for this and bring `SFC_WIND`back.)
